...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, user, credits }) => {
  ...
  const menuItems = [
    ...
    ...(FEATURE_FLAGS.CAROUSEL ? [{
      id: View.CAROUSEL, label: 'Criar Carrossel', icon: (
  ...
  return (
    ...
    >
      <div className="p-6 mb-4 flex items-center justify-center h-24">
        {/* AdGenius Logo Icon */}
        <div className={`transition-all duration-300 ${isExpanded ? 'w-full h-full' : 'w-10 h-10'} flex items-center justify-center relative`}>
          <img
            src={logo}
            alt="AdGenius"
      ...
      <nav className="flex-1 space-y-2 px-3">
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-4 p-3.5 rounded-xl transition-all duration-300 group relative overflow-hidden ${currentView === item.id
